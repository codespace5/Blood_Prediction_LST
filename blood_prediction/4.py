...
for i in range(2):
    ...
    # batch_size = 30
    model = Sequential()
    model.add(LSTM(units=30, return_sequences=True, input_shape=(1,look_back)))
    ...
    history = model.fit(X_train, Y_train, epochs=30, batch_size=7, validation_data=(X_test, Y_test), verbose=1, shuffle=False)
